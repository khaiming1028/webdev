...
<body>
    ...
    <main>
        <div class="container" style="margin-top: 120px;">
            <form method="POST" action="{{ route('forum.update', $forum->id) }}" class="my-form" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="form-group">
                    <label for="forums_title">Forum Title</label>
                    <input type="text" name="forums_title" id="forums_title" placeholder="Forum Title" class="form-control" value="{{ $forum->forums_title }}">
                </div>
                <div class="form-group">
                    <label for="forums_content">Forum Content</label>
                    <textarea name="forums_content" id="forums_content" placeholder="Forum Content" class="form-control" rows="5">{{ $forum->forums_content }}</textarea>
                </div>
